/**
 * ═══════════════════════════════════════════════════════════════
 *  APEX-MD  ·  binance.js  ·  Pro WebSocket Architecture
 *  ─────────────────────────────────────────────────────────────
 *  • Maintains in-memory kline cache for 15m, 1h, 4h per coin
 *  • Single combined WebSocket stream (no polling)
 *  • Exponential-backoff auto-reconnect
 *  • EventEmitter fires '15m_candle_close' when a 15m bar seals
...
// Timeframes streamed over WebSocket
const STREAM_TIMEFRAMES = ['15m', '1h', '4h'];

// How many candles to keep in memory per key
const CACHE_DEPTH = {
    '15m': 500,
    '1h':  100,
    '4h':   80,
...
function connectWebSocket() {
    ...
    console.log(`[WS] Connecting — ${_watchedCoins.length} coins × ${STREAM_TIMEFRAMES.length} TFs...`);
    ...
    _ws.on('open', () => {
        ...
        console.log(`[WS] ✅ Connected — streaming ${_watchedCoins.length * STREAM_TIMEFRAMES.length} kline streams.`);
    });
    ...
    _ws.on('message', (raw) => {
        try {
            ...
            if (!arr || arr.length === 0) {
                // Cache not yet seeded (startup race) — bootstrap with this frame
            ...
            if (lastRow[0] === row[0]) {
                // Same candle — update the live bar in place
                arr[arr.length - 1] = row;
            } else {
                // New candle opened — append and trim
                arr.push(row);
 ...
 *   2. Seed kline cache via REST for each coin × timeframe (once)
 *   3. Open the persistent WebSocket stream
 *   4. Schedule a light REST refresh for 5m and 1d data every 10 min
 *
...
async function initWebSocketStreams(limit = 30) {
    ...
    // 1. Discover top coins
    _watchedCoins = await getTopTrendingCoins(limit);
    console.log(`[WS] Coin list: ${_watchedCoins.join(', ')}`);

    // 2. Build seed task list
    const seedTasks = [];
    for (const coin of _watchedCoins) {
        for (const tf of STREAM_TIMEFRAMES) seedTasks.push({ coin, tf });
        seedTasks.push({ coin, tf: '5m' });
        seedTasks.push({ coin, tf: '1d' });
    }

    // Parallel batches of 8 with a 400 ms pause between batches
    const BATCH = 8;
    ...
    console.log('[WS] ✅ Historical kline cache seeded successfully.');
    _seedingDone = true;

    // 3. Open persistent WS
    connectWebSocket();

    // 4. Refresh non-streamed TFs (5m, 1d) every 10 minutes
    setInterval(async () => {
        for (const coin of _watchedCoins) {
            await seedCache(coin, '5m').catch(() => {});
 ...
 *
 * Returns kline data from the in-memory cache.
 * Falls back to a one-time REST call and caches the result if the
 * cache is empty (handles coins not in the top-30 stream, e.g. manual
 * .future commands for arbitrary coins).
 *
 * @param {string} coin       e.g. 'BTCUSDT'
 * @param {string} timeframe  e.g. '15m'
 * @param {number} limit      candles to return (from the tail)
 */
async function getKlineDataFromCache(coin, timeframe, limit = 100) {
    ...
    // Cache miss — one-time REST fetch
...
async function getKlineData(coin, timeframe, limit = 100) {
    return await withRetry(async () => {
        ...
        console.log('Binance REST Error: ', error.message);
...
module.exports = {
    ...
    // ── Cache-based data (use inside bot) ──
