...
async function getTopTrendingCoins(limit = 20) {
    // ✅ UPGRADED: Much better coin quality filter for scanner
    // Old: just sorted by volume → included stablecoins, dead coins, zero-momentum coins
    // New: multiple quality filters → only volatile, liquid, moving coins
    const STABLECOINS = new Set([
        'USDCUSDT','BUSDUSDT','DAIUSDT','TUSDUSDT','USDPUSDT','FRAXUSDT',
        'LUSDUSDT','USTCUSDT','EURUSDT','GBPUSDT','FDUSDUSDT','PYUSDUSDT',
        'USDDUSDT','SUSDUSDT','GUSDUSDT','USDXUSDT'
    ]);
    try {
        const res = await axios.get('https://api.binance.com/api/v3/ticker/24hr');
        let coins = res.data.filter(c => {
            if (!c.symbol.endsWith('USDT')) return false;
            if (STABLECOINS.has(c.symbol)) return false;                     // No stablecoins
            if (c.symbol.includes('UP') || c.symbol.includes('DOWN')) return false; // No leveraged tokens
            if (c.symbol.includes('BULL') || c.symbol.includes('BEAR')) return false;
            
            const price     = parseFloat(c.lastPrice);
            const volume24h = parseFloat(c.quoteVolume);  // USDT volume
            const change24h = Math.abs(parseFloat(c.priceChangePercent));
            const count     = parseInt(c.count || 0);     // trade count
            
            if (price < 0.00001) return false;            // Ultra-micro cap → noise
            if (volume24h < 10_000_000) return false;     // Min $10M 24h vol (liquid enough)
            if (change24h < 0.5) return false;            // Min 0.5% move (has momentum)
            if (change24h > 50) return false;             // >50% pump = manipulation, skip
            if (count < 5000) return false;               // Min trade frequency
            
            return true;
        });
        
        // Sort by: volume AND change combined (find volatile + liquid coins)
        coins.sort((a, b) => {
            const scoreA = parseFloat(a.quoteVolume) * Math.min(Math.abs(parseFloat(a.priceChangePercent)), 15);
            const scoreB = parseFloat(b.quoteVolume) * Math.min(Math.abs(parseFloat(b.priceChangePercent)), 15);
