...
function checkCandlePattern(candles) {
    ...
    const lBody = Math.abs(lClose - lOpen), lLowerWick = isLGreen ? lOpen - lLow : lClose - lLow, lUpperWick = isLGreen ? lHigh - lClose : lHigh - lOpen;
    if (lLowerWick > lBody * 2 && lUpperWick < lBody * 0.5) return "Hammer 🟢";
    if (lUpperWick > lBody * 2 && lLowerWick < lBody * 0.5) return "Shooting Star 🔴";
    return "Neutral";
}

// 🚀 අලුතින් එකතු කළ Volume Profile (Point of Control)
...
module.exports = { calculateRSI, calculateEMA, calculateATR, checkDivergence, checkCandlePattern, calculatePOC };
