...
// Most popular single trend indicator. Green = Bull, Red = Bear.
// Gives cleaner signals than EMA alone.
// ============================================================
function calculateSupertrend(candles, period = 10, multiplier = 3.0) {
    if (candles.length < period + 2) return { signal: 'NEUTRAL', trend: 'NEUTRAL', display: '⚪ Supertrend N/A' };

    // Calculate ATR for each candle (Wilder's smoothed)
    const trArr = [];
    for (let i = 1; i < candles.length; i++) {
    ...
    // Supertrend bands
    let upBand = [], downBand = [], supertrend = [], direction = [];
    const startIdx = period; // align with atrArr

    for (let i = 0; i < atrArr.length; i++) {
        const ci = i + 1; // candles index (shifted by 1 for prev close)
        const hl2 = (parseFloat(candles[ci][2]) + parseFloat(candles[ci][3])) / 2;
        upBand.push(hl2 + multiplier * atrArr[i]);
        downBand.push(hl2 - multiplier * atrArr[i]);
    }

    // Compute supertrend line
    let prevUp = upBand[0], prevDown = downBand[0], prevST = downBand[0], prevDir = 1;
    const stArr = [], dirArr = [];
    for (let i = 0; i < upBand.length; i++) {
        const ci = i + 1;
        const close = parseFloat(candles[ci][4]);

        const finalUp = upBand[i] < prevUp || parseFloat(candles[ci-1 >= 0 ? ci-1 : 0][4]) < prevUp
            ? upBand[i] : prevUp;
        const finalDown = downBand[i] > prevDown || parseFloat(candles[ci-1 >= 0 ? ci-1 : 0][4]) > prevDown
            ? downBand[i] : prevDown;

        let dir, st;
        if (prevST === prevUp) {
            dir = close > finalUp ? 1 : -1;
        } else {
            dir = close < finalDown ? -1 : 1;
        }
        st = dir === 1 ? finalDown : finalUp;
    ...
    if (stArr.length < 2) return { signal: 'NEUTRAL', trend: 'NEUTRAL', display: '⚪ Supertrend N/A' };
    ...
    let signal = isBull ? 'BULL' : 'BEAR';
    let display;
    if (justFlipUp)   display = `🟢🟢 *SUPERTREND FLIP UP* — Strong Bull Signal! ST: $${lastST.toFixed(4)}`;
    else if (justFlipDown) display = `🔴🔴 *SUPERTREND FLIP DOWN* — Strong Bear Signal! ST: $${lastST.toFixed(4)}`;
    else if (isBull)  display = `🟢 Supertrend Bull (above $${lastST.toFixed(4)})`;
    else              display = `🔴 Supertrend Bear (below $${lastST.toFixed(4)})`;

    return {
        signal, isBull, isBear, justFlipUp, justFlipDown,
        supertrendLevel: lastST.toFixed(4),
        display
    };
}

// ============================================================
// ✅ NEW: RVOL - Relative Volume (actual vs 20-period average)
// RVOL > 2.0 = Institutional activity / major move likely
// RVOL < 0.5 = Low interest / avoid entries
// ============================================================
function calculateRVOL(candles) {
    if (candles.length < 22) return { rvol: 1.0, signal: 'NORMAL', display: '⚪ RVOL N/A' };
    ...
    let signal, display, isBull = false, isBear = false;
    if (rvol >= 3.0)       { signal = 'EXTREME'; display = `🔥🔥 RVOL ${rvol.toFixed(1)}x (Extreme! Institutional move)`; }
    else if (rvol >= 2.0)  { signal = 'HIGH';    display = `🔥 RVOL ${rvol.toFixed(1)}x (High vol — trust the move)`; }
    ...
    else                   { signal = 'LOW';     display = `⚠️ RVOL ${rvol.toFixed(1)}x (Low vol — wait for confirmation)`; }

    const isTrustworthy = rvol >= 1.3;
    return { rvol: parseFloat(rvol.toFixed(2)), signal, isTrustworthy, display };
}

// ============================================================
// ✅ NEW: MTF MACD Confluence (15m + 1H same direction)
// Much stronger signal than single timeframe MACD
// ============================================================
function checkMTFMACD(candles15m, candles1H) {
    if (!candles15m || !candles1H) return { signal: 'NEUTRAL', display: '⚪ MTF MACD N/A' };

    function getMACDDir(candles) {
        if (candles.length < 35) return 'NEUTRAL';
        const ema12 = candles.slice(-30).map((_, i, arr) => parseFloat(arr[i][4]));
        const ema26arr = candles.map(c => parseFloat(c[4]));
        
        // Simple: compare last 2 MACD values for direction
        const k12 = 2 / 13, k26 = 2 / 27;
        let e12 = ema12[0], e26 = ema26arr[0];
        for (let i = 1; i < ema12.length; i++) {
            e12 = ema12[i] * k12 + e12 * (1 - k12);
        }
        for (let i = 1; i < ema26arr.length; i++) {
            e26 = ema26arr[i] * k26 + e26 * (1 - k26);
        }
        const macd = e12 - e26;
