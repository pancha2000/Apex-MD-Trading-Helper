/**
 * ═══════════════════════════════════════════════════════════════
 *  APEX-MD  ·  analyzer.js  ·  WebSocket Cache Edition
 *  ─────────────────────────────────────────────────────────────
 *  All getKlineData() REST calls replaced with
 *  binance.getKlineDataFromCache() — zero polling, instant data
 *  from the in-memory WebSocket cache maintained by binance.js.
 * ═══════════════════════════════════════════════════════════════
 */
 ...
 *
 * DATA SOURCE: All kline data is now served from the in-memory
...
async function run14FactorAnalysis(coin, timeframe = '15m') {
    ...
    // ── 1. Data Fetching (from WS cache, not REST polling) ──────
    const [currentCandles, candles5m, candles1H, candles4H, candlesDaily] = await Promise.all([
        ...
        binance.getKlineDataFromCache(coin, '5m',     50),
        binance.getKlineDataFromCache(coin, '1h',     60),
        binance.getKlineDataFromCache(coin, '4h',     60),
        binance.getKlineDataFromCache(coin, '1d',     10).catch(() => null),
    ]);
    ...
    // v4 precision — liquidity sweep + ChoCH
