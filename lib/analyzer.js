...
async function run14FactorAnalysis(coin, timeframe = '15m') {
    ...
    const [currentCandles, candles5m, candles1H, candles4H, candlesDaily] = await Promise.all([
        ...
        binance.getKlineDataFromCache(coin, '5m',     500),   // ← 500 candles, live WS stream
        binance.getKlineDataFromCache(coin, '1h',     60),
        binance.getKlineDataFromCache(coin, '4h',     80),    // ← extra depth for 4H OB detection
    ...
    return {
        ...
        // ── Scoring ──
        score: finalScore, maxScore: 55, reasons: finalReasons,   // maxScore bumped 50→55 for 5m bonus
