...
async function run14FactorAnalysis(coin, timeframe = '15m') {
    ...
    // ✅ NEW v4: RVOL - only count if strong volume (confirms moves)
    if (rvol.signal === 'EXTREME') { longScore += 0.5; shortScore += 0.5; longR.push("RVOL Extreme 🔥"); shortR.push("RVOL Extreme 🔥"); }
    else if (rvol.signal === 'HIGH') { longScore += 0.5; shortScore += 0.5; longR.push("RVOL High 🔥"); shortR.push("RVOL High 🔥"); }
    ...
    return {
        ...
        score: finalScore, maxScore: 30, reasons: finalReasons,  // v4: +Supertrend +RVOL +MTFMacd  // maxScore: EMA ribbon adds up to +2
