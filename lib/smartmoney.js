// ============================================================
// ✅ FIXED smartmoney.js - 4 Decimal Precision Update
...
// දශමස්ථාන 4 කට (4 Decimal Places) යාවත්කාලීන කර ඇත
// ============================================================
    ...
    for (let i = candles.length - 15; i < candles.length - 2; i++) {
        ...
        if (isRed &&
            ...
            bullishOB = {
                ...
                // SL = zone bottom - small buffer (zone invalidation)
                sl: (bottom * 0.998).toFixed(4)
            };
        }
        ...
        if (isGreen &&
            ...
            bearishOB = {
                ...
                mid: mid.toFixed(4),
                display: `$${bottom.toFixed(4)} - $${top.toFixed(4)}`,
                // SL = zone top + small buffer
                sl: (top * 1.002).toFixed(4)
            };
        }
    }
...
function selectBestEntry(currentPrice, ob, fib618, poc, vwapPrice, direction, atr) {
    ...
    if (direction === 'LONG') {
        if (ob) {
            candidates.push({ name: "Bullish OB Midpoint", price: parseFloat(ob.mid), zoneBottom: parseFloat(ob.bottom), zoneTop: parseFloat(ob.top), sl: parseFloat(ob.sl), priority: 1 });
        }
        if (fib618 && parseFloat(fib618) < price * 1.01) {
            candidates.push({ name: "Fib 61.8%", price: parseFloat(fib618), zoneBottom: parseFloat(fib618) - atrVal * 0.3, zoneTop: parseFloat(fib618) + atrVal * 0.3, sl: (parseFloat(fib618) - atrVal * 1.5), priority: 2 });
        }
        if (poc && parseFloat(poc) < price * 1.01) {
            candidates.push({ name: "Point of Control (POC)", price: parseFloat(poc), zoneBottom: parseFloat(poc) - atrVal * 0.3, zoneTop: parseFloat(poc) + atrVal * 0.3, sl: (parseFloat(poc) - atrVal * 1.5), priority: 3 });
        }
        if (vwapPrice && vwapPrice > 0 && vwapPrice < price * 1.01) {
            candidates.push({ name: "VWAP Support", price: vwapPrice, zoneBottom: vwapPrice - atrVal * 0.2, zoneTop: vwapPrice + atrVal * 0.2, sl: (vwapPrice - atrVal * 1.5), priority: 4 });
        }

        if (candidates.length === 0) {
            return { name: "Current Price (No Zone)", price: price, zoneBottom: price - atrVal * 0.5, zoneTop: price, sl: price - atrVal * 1.5, priority: 5, warning: "⚠️ Strong zone නොමැත. Entry risk ඉහළයි." };
        }
    ...
    if (direction === 'SHORT') {
        if (ob) {
            candidates.push({ name: "Bearish OB Midpoint", price: parseFloat(ob.mid), zoneBottom: parseFloat(ob.bottom), zoneTop: parseFloat(ob.top), sl: parseFloat(ob.sl), priority: 1 });
        }
        if (fib618 && parseFloat(fib618) > price * 0.99) {
            candidates.push({ name: "Fib 61.8% Resistance", price: parseFloat(fib618), zoneBottom: parseFloat(fib618) - atrVal * 0.3, zoneTop: parseFloat(fib618) + atrVal * 0.3, sl: (parseFloat(fib618) + atrVal * 1.5), priority: 2 });
        }
        if (poc && parseFloat(poc) > price * 0.99) {
            candidates.push({ name: "POC Resistance", price: parseFloat(poc), zoneBottom: parseFloat(poc) - atrVal * 0.3, zoneTop: parseFloat(poc) + atrVal * 0.3, sl: (parseFloat(poc) + atrVal * 1.5), priority: 3 });
        }

        if (candidates.length === 0) {
            return { name: "Current Price (No Zone)", price: price, zoneBottom: price, zoneTop: price + atrVal * 0.5, sl: price + atrVal * 1.5, priority: 5, warning: "⚠️ Strong zone නොමැත. Entry risk ඉහළයි." };
        }
...
function getOrderTypeSuggestion(entryPrice, currentPrice, direction) {
    ...
    if (direction === 'LONG') {
        if (entry >= current * 0.999) return { type: "MARKET ORDER 🟢", reason: "Entry price දැනට current price ට ළඟයි. Market Order use කළ හැකිය." };
        else if (diffPct <= 2) return { type: "LIMIT ORDER ⏳", reason: `Entry ($${entry}) current ($${current}) ට ${diffPct.toFixed(2)}% පහළ. Limit Order set කරන්න.` };
        else return { type: "LIMIT ORDER ⏳ (Patience)", reason: `Entry ($${entry}) current ($${current}) ට ${diffPct.toFixed(2)}% දුරයි. OB zone retest වෙනකල් wait කරන්න.` };
    }
    if (direction === 'SHORT') {
        if (entry <= current * 1.001) return { type: "MARKET ORDER 🔴", reason: "Entry price දැනට current price ට ළඟයි. Market Order use කළ හැකිය." };
        else if (diffPct <= 2) return { type: "LIMIT ORDER ⏳", reason: `Entry ($${entry}) current ($${current}) ට ${diffPct.toFixed(2)}% ඉහළ. Limit Order set කරන්න.` };
        else return { type: "LIMIT ORDER ⏳ (Patience)", reason: `Entry ($${entry}) current ($${current}) ට ${diffPct.toFixed(2)}% දුරයි. OB zone retest වෙනකල් wait කරන්න.` };
    }
...
// ============================================================
// ✅ Main SMC Analysis Function (දශම 4 කට)
// ============================================================
function analyzeSMC(candles) {
    ...
    let fib618 = (maxPrice - (diff * 0.618)).toFixed(4);
    let fib786 = (maxPrice - (diff * 0.786)).toFixed(4);
    let ext1618 = (maxPrice + (diff * 0.618)).toFixed(4);
    let ext2618 = (maxPrice + (diff * 1.618)).toFixed(4);
    let extMinus1618 = (minPrice - (diff * 0.618)).toFixed(4);

    let recentCandles = candles.slice(-15);
    let swingHigh = Math.max(...recentCandles.map(c => parseFloat(c[2]))).toFixed(4);
    let swingLow = Math.min(...recentCandles.map(c => parseFloat(c[3]))).toFixed(4);
