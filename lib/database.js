...
// 🚀 Settings වලට අලුත් strictMode එකතු කළා
const botSettingsSchema = new mongoose.Schema({
    ...
    strictMode: { type: Boolean, default: true } // 👈 අලුත් Setting එක (මුලින්ම ON වෙලා තියෙන්නේ)
});
...
async function saveMargin(jid, amount) {
    try { return await User.findOneAndUpdate({ jid }, { margin: amount }, { new: true, upsert: true }); } catch (error) { return null; }
}
async function getMargin(jid) {
    try { const user = await User.findOne({ jid }); return user ? user.margin : 0; } catch (error) { return 0; }
}
...
async function getSettings() {
    try {
        ...
        return { autoSignal: false, trailingSl: true, trendFilter: true, strictMode: true }; 
    }
}

async function updateSettings(data) {
    try { return await BotSettings.findOneAndUpdate({ id: 'global' }, data, { new: true, upsert: true }); } catch (error) { return null; }
}
