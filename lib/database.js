...
const TradeSchema = new mongoose.Schema({
    ...
    staleWarned: { type: Boolean, default: false }, // ✅ BUG 5 FIX: persisted stale warning flag
});

// ─── MODELS ───
// ✅ FIX: OverwriteModelError — use existing model if already compiled.
// This happens when database.js AND binance.js both register the same models,
// or when the module is hot-reloaded after a crash.
...
// Only the FINAL close call (or a full close) passes countTrade=true (default).
...
async function getTradeStats(jid) {
    ...
    for (let i = 0; i < closedTrades.length; i++) {
        ...
        // Old code: never counted loss streaks, reset count to 0 on loss
