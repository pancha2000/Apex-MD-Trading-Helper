...
async function getCVD(coin) {
    try {
        ...
        if (!trades || trades.length === 0) {
            return { signal: 'NEUTRAL', emoji: '⚪', display: 'N/A', detail: 'No trade data' };
        }
...
async function getHTFLevels(coin) {
    try {
        ...
        if (!weeklyCandles || !monthlyCandles) {
            return { signal: 'NEUTRAL', emoji: '⚪', display: 'N/A', detail: 'HTF data unavailable' };
        }
...
async function getBTCCorrelation(coin, tradeDirection) {
    try {
        // If it IS BTC, skip
        if (coin === 'BTCUSDT') {
            return { signal: 'N/A', emoji: '⚪', display: 'N/A (IS BTC)', detail: 'Trade is BTC itself', isAlts: false };
        }
        ...
        if (!btcData || !coinData) {
            return { signal: 'NEUTRAL', emoji: '⚪', display: 'N/A', detail: 'API error' };
        }
...
async function getPutCallRatio(coin) {
    try {
        ...
        if (!['BTC', 'ETH'].includes(base)) {
            return { signal: 'N/A', emoji: '⚪', display: 'N/A (No options)', detail: 'Options only for BTC/ETH' };
        }
        ...
        if (!data || !data.result) {
            return { signal: 'NEUTRAL', emoji: '⚪', display: 'N/A', detail: 'Deribit API unavailable' };
        }
        ...
        if (totalCallOI === 0) {
            return { signal: 'NEUTRAL', emoji: '⚪', display: 'N/A', detail: 'No options data' };
        }
...
async function getNetflowProxy(coin) {
    try {
        ...
        if (totalVol === 0) return { signal: 'NEUTRAL', emoji: '⚪', display: 'N/A', detail: 'No volume' };
...
async function getSocialVolume(coin, lunarApiKey = null) {
    if (!lunarApiKey) {
        return {
            signal: 'N/A', emoji: '⚪', display: 'N/A (No LUNAR_API)',
    ...
    try {
        ...
        if (!data || !data.data) {
            return { signal: 'NEUTRAL', emoji: '⚪', display: 'N/A', detail: 'LunarCrush unavailable' };
        }
